...
class UserPreferences @Inject constructor(
    ...
    // BUG FIX: the old flow generated a new UUID on every emission without persisting it.
    // Now userId is a derived flow that reads the stored value only;
    // call ensureUserId() at startup to guarantee the ID exists in storage.
    val userId: Flow<String> = context.dataStore.data.map { prefs ->
        prefs[KEY_USER_ID] ?: ""
    }

    suspend fun ensureUserId(): String {
        return context.dataStore.updateData { prefs ->
            if (prefs[KEY_USER_ID] == null) {
                prefs.toMutablePreferences().apply {
                    set(KEY_USER_ID, UUID.randomUUID().toString())
                }
            } else prefs
        }[KEY_USER_ID]!!
    }
